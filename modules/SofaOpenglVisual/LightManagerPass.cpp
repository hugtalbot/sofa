...
SOFA_DECL_CLASS(LightManagerPass)
//Register LightManager in the Object Factory
int LightManagerPassClass = core::RegisterObject("LightManagerPass")
        .add< LightManagerPass >()
        ;

LightManagerPass::LightManagerPass()
...
void LightManagerPass::initVisual()
{
...
void LightManagerPass::putLight(Light::SPtr light)
{
...
void LightManagerPass::putLights(std::vector<Light::SPtr> lights)
{
    for (std::vector<Light::SPtr>::iterator itl = lights.begin(); itl != lights.end() ; ++itl)
        putLight(*itl);
}

void LightManagerPass::makeShadowMatrix(unsigned int i)
{
...
void LightManagerPass::fwdDraw(core::visual::VisualParams* vp)
{
...
void LightManagerPass::bwdDraw(core::visual::VisualParams* )
{
#ifdef SOFA_HAVE_GLEW
    for(unsigned int i=0 ; i<lights.size() ; ++i)
    {
...
void LightManagerPass::draw(const core::visual::VisualParams* )
{
}

void LightManagerPass::clear()
{
    for (unsigned int i=0 ; i<MAX_NUMBER_OF_LIGHTS ; ++i)
        glDisable(GL_LIGHT0+i);
    lights.clear();
...
void LightManagerPass::preDrawScene(VisualParams* vp)
{
...
//    glEnable(GL_DEPTH_BUFFER_BIT);
    ...
    for (std::vector<Light::SPtr>::iterator itl = lights.begin(); itl != lights.end() ; ++itl)
            {
                   // std::cout<<"light : "<<(*itl)->getPathName()<<std::endl;
...
bool LightManagerPass::drawScene(VisualParams* /*vp*/)
{
    return false;
}

void LightManagerPass::postDrawScene(VisualParams* vp)
{
    restoreDefaultLight(vp);
    prerendered=false;
}

void LightManagerPass::restoreDefaultLight(VisualParams* vp)
{
...
void LightManagerPass::handleEvent(sofa::core::objectmodel::Event* event)
{
